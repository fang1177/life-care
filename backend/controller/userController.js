...
export const addNewDoctor = catchAsyncErrors(async (req, res, next) => {
  ...
  if (useExistingPhoto === "true" && selectedPhoto) {
    ...
    cloudinaryResponse = {
      ...
      secure_url: `${process.env.NODE_ENV === "development" ? "http://localhost:4000" : "/dashboard"}${selectedPhoto}`
    };
  } else {
    // Upload new photo
...
export const updateDoctor = catchAsyncErrors(async (req, res, next) => {
  console.log("Update doctor request received for ID:", req.params.id);
  ...
  if (useExistingPhoto === "true" && selectedPhoto) {
    ...
    console.log("Uploading new photo:", req.files.docAvatar.name);
    // Upload new photo
    docAvatar = req.files.docAvatar;
    const allowedFormats = ["image/png", "image/jpeg", "image/webp"];
    if (!allowedFormats.includes(docAvatar.mimetype)) {
    ...
    console.log("No photo provided for update");
  }
